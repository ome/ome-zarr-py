...
if __name__ == "__main__":
    ...
    tw.group = tw.root.create_group("test")

    tw.path_v3 = tw.path / "v3"
    store_v3 = parse_url(tw.path_v3, mode="w").store
    root_v3 = zarr.group(store=store_v3)
    tw.group_v3 = root_v3.create_group("test")

    # # call the specific test case: 5D shape, FormatV05, np.array, storage_options_list=True
    # # This matches the parametrized node id like:
    # # tests/test_writer.py::TestWriter::test_writer[V05-np.array-True]
    # tw.test_default_compression(
    #     array_constructor=da.from_array,
    #     format_version=FormatV05,
    # )
    # tw.test_write_image_dask(
    #     read_from_zarr=True,
    #     compute=True,
    #     zarr_format=3
    # )
    ...
    #     storage_options_list=True,
    # )

    tw.test_write_image_current(
        array_constructor=np.array,
        zarr_format=3,
    )
    ...
    # os.mkdir(path / "data")
    # tw.path = pathlib.Path(tmp.name) / "data"
    # # create zarr v2 group...
    # tw.store = parse_url(tw.path, mode="w", fmt=FormatV04()).store
    # tw.root = zarr.group(store=tw.store)
    # # create zarr v3 group...
    # tw.path_v3 = tw.path / "v3"
    # store_v3 = parse_url(tw.path_v3, mode="w").store
    # tw.root_v3 = zarr.group(store=store_v3)

    # # tw.test_write_labels(
    # #     shape=(1, 2, 1, 256, 256),
    # #     format_version=FormatV05,
    # #     array_constructor=np.array,
    # #     scale_type="noop",
    ...
    # tw.test_write_multiscale_labels(
    #     shape=(3, 256, 256),
